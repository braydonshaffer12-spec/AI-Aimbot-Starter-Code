...
def gameSelection(settings: Settings) -> (bettercam.BetterCam, int, Union[int, None]):
    ...
    left = ((videoGameWindow.left + videoGameWindow.right) // 2) - (settings.screenShotWidth // 2)
    top = videoGameWindow.top + \
        (videoGameWindow.height - settings.screenShotHeight) // 2
    right, bottom = left + settings.screenShotWidth, top + settings.screenShotHeight

    region: tuple = (left, top, right, bottom)

    # Calculating the center Autoaim box
    cWidth: int = settings.screenShotWidth // 2
    cHeight: int = settings.screenShotHeight // 2
